...
export const packageService = {
  async getPackages(page = 0, size = 20): Promise<PaginatedResponse<Package>> {
    const { data } = await api.get<PaginatedResponse<Package>>("/packages", { params: { page, size } });
    return data;
  },
  ...
  async createPackage(request: CreatePackageRequest): Promise<Package> {
    const { data } = await api.post<Package>("/packages", request);
    return data;
  },

  async updateStatus(id: string, request: UpdateStatusRequest): Promise<Package> {
    const { data } = await api.patch<Package>(`/packages/${id}/status`, request);
    return data;
  },

  async updateTracking(id: string, request: UpdateTrackingRequest): Promise<Package> {
    const { data } = await api.patch<Package>(`/packages/${id}/tracking`, request);
    return data;
  },
